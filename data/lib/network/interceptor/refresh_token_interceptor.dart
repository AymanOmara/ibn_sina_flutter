...
import 'package:data/features/authentication/model/refresh_token.dart';
import 'package:data/features/authentication/request/refresh_token_request.dart';
...
class RefreshTokenInterceptor extends Interceptor {
  ...
  FutureOr refreshToken(
    ...
    if (refreshToken?.hashCode == 401) {
      throw RefreshTokenException();
    }
    dio.options.headers.addAll({"Authorization": "Bearer $newToken"});
    userLocal.setAccessToken(refreshToken?.token ?? "");
    userLocal.setRefreshToken(refreshToken?.refreshToken ?? "");
    isRefreshing = false;
    failedRequests.add({'err': err, 'handler': handler});
